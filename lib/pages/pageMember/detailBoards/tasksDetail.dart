...
class _TasksdetailPageState extends State<TasksdetailPage> {
  ...
  void dispose() {
    ...
    _descriptionController.dispose(); // เพิ่มบรรทัดนี้
    _descriptionFocusNode.dispose(); // เพิ่มบรรทัดนี้
  ...
  String _getHeaderTitle() {
    ...
    if (combinedData.containsKey('task')) {
      return combinedData['task']['taskName'] ?? 'ไม่มีชื่อ Task';
    }
  ...
  Future<void> _updateTaskStatus(bool isGroupTask, String newStatus) async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        // รีเฟรชข้อมูลตามประเภทของ task
        if (isGroupTask) {
          // อาจจะมีฟังก์ชันพิเศษสำหรับโหลดข้อมูล group task
          await loadDataAsync(); // หรือ loadGroupTaskData() ถ้ามี
        } else {
          await loadDataAsync();
        }
      } else {
  ...
  Future<void> _assignUserToTask(
    ...
    try {
      ...
      if (response.statusCode == 201) {
        ...
        // รอให้โหลดข้อมูลเสร็จก่อน และส่ง callback เพื่อให้ popup อัปเดต
        await loadDataAsync(
          onDataLoaded: () {
            // เพิ่มการตรวจสอบก่อนเรียก setModalState
            if (mounted) {
              try {
                setModalState(() {
                  // trigger rebuild ของ popup หลังจากข้อมูลใหม่มาแล้ว
                });
              } catch (e) {
        ...
        log('❌ Error assigning user: ${response.statusCode}');
  ...
  Future<void> _deleteAssignedUser(
    int assId,
    StateSetter setModalState,
    Map<int, bool> selectedMap, // เพิ่ม parameter นี้
    int userId, // เพิ่ม userId เพื่อรีเซ็ต
  ) async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        // รีเซ็ต isSelected ให้กลับเป็น false
        setModalState(() {
          selectedMap[userId] = false;
        });
  ...
  // สร้างแท็ป description, checklist, file
  ...
  // Helper function เพื่อลบ temp checklist
  ...
  Future<void> _showDeleteFileDialog(
    ...
    if (currentTask != null) {
      index = _findIndexCurrentTask(currentTask);
      // 🔧 วิธีที่ถูกต้องในการดึง taskId สำหรับ group task
      if (isgroupTask == true && combinedData?['attachments'] != null) {
        // สำหรับ Group Task: พยายามหา taskId จาก attachment item ใดๆ
        ...
        // สำหรับ Individual Task: ใช้ taskId จาก currentTask หรือจาก attachment
  ...
  // แปลงเวลาที่ใช้แสดงในfile
  String _formatUploadDate(String uploadAt) {
    try {
      DateTime dateTime = DateTime.parse(uploadAt);
      return '${dateTime.day}/${dateTime.month}/${dateTime.year}';
    } catch (e) {
      return uploadAt; // ถ้า parse ไม่ได้ให้แสดงข้อมูลเดิม
    }
  }

  // ฟังก์ชันช่วยสำหรับคำนวณขนาดไฟล์
              ...
              actions: [
                TextButton(
                  onPressed: () {
                    ...
                    _uploadFile(isgroupTask); // กลับไปที่ popup หลัก
                  },
                  child: Text('Back'),
  ...
  // ฟังก์ชันกำหนดประเภทไฟล์จาก extension
  ...
  // ฟังก์ชันกำหนดประเภทภาพจาก extension
